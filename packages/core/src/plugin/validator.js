 ...
 */
export function validatePlugin(config) {
    if (!config || typeof config !== "object") {
        throw new PluginValidationError("Plugin config must be a non-null object", "unknown");
    }
    const c = config;
    if (typeof c.name !== "string" || c.name.length === 0) {
        throw new PluginValidationError("Plugin must have a non-empty string 'name'.\n\nA valid plugin must export a definePlugin() result:\n\n  export default definePlugin({\n    name: \"my-plugin\",\n    commands: [...],\n  });", String(c.name ?? "unknown"));
    }
    if (!NAME_PATTERN.test(c.name)) {
        throw new PluginValidationError(`Plugin name "${c.name}" is invalid. Must be lowercase alphanumeric and hyphens, cannot start with hyphen.`, c.name);
    }
    if (typeof c.version !== "string" || !valid(c.version)) {
        throw new PluginValidationError(`Plugin "${c.name}" has invalid version "${String(c.version)}". Must be valid semver.`, c.name);
    }
 ...
 */
export function validateSeedcliVersion(plugin, runtimeVersion) {
    if (!plugin.seedcli)
        return;
    if (!satisfies(runtimeVersion, plugin.seedcli)) {
        throw new PluginValidationError(`Plugin "${plugin.name}" requires Seed CLI ${plugin.seedcli}\n\n  Current Seed CLI version: ${runtimeVersion}\n  Required by plugin:       ${plugin.seedcli}\n\n  Upgrade Seed CLI:\n    bun update @seedcli/core`, plugin.name);
    }
}
/**
 ...
 */
export function validatePeerDependencies(plugin, loaded) {
    if (!plugin.peerPlugins)
        return;
    for (const [peerName, versionRange] of Object.entries(plugin.peerPlugins)) {
        const peer = loaded.get(peerName);
        if (!peer) {
            throw new PluginDependencyError(`Plugin "${plugin.name}" requires peer plugin "${peerName}" but it is not loaded.\n\n  Install the required plugin:\n    bun add @mycli/plugin-${peerName}`, plugin.name, peerName);
        }
        if (!satisfies(peer.version, versionRange)) {
            throw new PluginDependencyError(`Plugin "${plugin.name}" requires "${peerName}@${versionRange}" but found "${peer.version}".\n\n  Upgrade the plugin:\n    bun update @mycli/plugin-${peerName}`, plugin.name, peerName);
        }
    }
}
