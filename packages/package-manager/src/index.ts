export { getCommands } from "./commands.js";
export { detect } from "./detect.js";
export { create } from "./manager.js";
export type {
	InstallOptions,
	PackageManager,
	PackageManagerModule,
	PackageManagerName,
	RunOptions,
} from "./types.js";

import { detect } from "./detect.js";
import { create } from "./manager.js";
import type { InstallOptions, RunOptions } from "./types.js";

export async function install(packages: string[], options?: InstallOptions): Promise<void> {
	const pm = await create(await detect(options?.cwd));
	await pm.install(packages, options);
}

export async function installDev(packages: string[], options?: InstallOptions): Promise<void> {
	const pm = await create(await detect(options?.cwd));
	await pm.installDev(packages, options);
}

export async function remove(packages: string[], options?: InstallOptions): Promise<void> {
	const pm = await create(await detect(options?.cwd));
	await pm.remove(packages, options);
}

export async function run(script: string, options?: RunOptions): Promise<void> {
	const pm = await create(await detect(options?.cwd));
	await pm.run(script, options);
}
